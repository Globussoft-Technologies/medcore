...
### Added
- **2026-05-05 office session — release.yml aggressive sharding (2 commits) + eslint 10 migration PR (#663) + bookkeeping reconcile.** release.yml restructured from a 2-job monolith (which routinely hit the 60-min job timeout) into 23 parallel jobs targeting ≤10 min total wall-clock. Topology: 1 typecheck + 1 api-tests-fast + 4 api-tests-integration shards (`vitest --shard=N/4`) + 1 web-tests-full + 8 Chromium E2E shards + 8 WebKit E2E shards + new `merge-reports` job that stitches the 16 blob reports into one Playwright HTML. Iteration history documented in the file-header comment block: v1 (`338088b`) was 6 jobs total; v2 (`9b2291a`) is 23 jobs. Headroom for v3 (build-prebuild + ms-playwright cache) annotated. `release-summary` `needs[]` updated. Per-shard server-log artifacts (separate per shard) make failure triage cheaper. **Eslint 9→10 migration** lives on PR #663 (branch `fix/eslint-10-migration`, commit `9126a86`); migrates `apps/web` from deprecated `next lint` to direct ESLint CLI with `eslint.config.mjs` flat-config; Lint CI green. **CLAUDE.md "Cron learnings" bookkeeping reconcile** — 6 RIPE bullets moved Open→Promoted with closing-commit ref `e3166f0` (the 5 skill edits had already shipped that morning; the move-from-Open lagged). Only "Cross-patient test fixture identity-mismatch" stays Open (1 instance). **`.npmrc`** verified-still-required: removability check trips ERESOLVE on a NEW conflict shape (`apps/mobile/node_modules/react-dom@18.3.1` lingering from before #471/#467 react@19 bump); fix path documented in comment block.
