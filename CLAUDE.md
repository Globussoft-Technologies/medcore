...
- 2026-05-05 cron-tick (commits `36b6532` + `e8c648d`): **Page-shape "admin-gate placeholder" — third archetype alongside redirect-bounce + universal-access — RIPE NOW (2 instances)**. (1) `apps/web/src/app/dashboard/ai-kpis/page.tsx:296` renders `data-testid="ai-kpis-admin-gate"` for non-ADMIN. (2) `apps/web/src/app/dashboard/ai-fraud/page.tsx:478-487` renders an inline `<ShieldCheck>` "Restricted" placeholder for non-{ADMIN,RECEPTION} via `canRead` flag (page.tsx:382). Both pages keep the page bundle mounted + outer chrome rendered, but data panels are replaced. CLAUDE.md gotcha #7 codified only 2 archetypes ("VIEW_ALLOWED redirect" + "no client gate, API-only"); this is a confirmed 3rd. **Variation worth capturing in the skill**: ai-kpis exposes a dedicated gate-testid (`ai-kpis-admin-gate`) — the cleanest test contract; ai-fraud reuses its outer-wrapper testid (`ai-fraud-page`) for both allowed and gate branches — test must distinguish via inner content (the "Restricted" copy or absence of data-panel testids). **Suggested skill extension** (worth promoting now): extend `/medcore-e2e-spec` with a "page-shape decision matrix" sub-section enumerating all 3 archetypes + a recommendation that NEW pages adopting this shape SHOULD use a dedicated gate-testid (`<route>-admin-gate` convention) for testability — the ai-fraud reuse-the-wrapper variant is harder to assert without depending on textual copy. **Status: RIPE for promotion** (2 confirming instances + identical structural fix pattern + concrete testid-naming recommendation).
