...
- 2026-05-05 cron-tick (commits `e9e032a` + `d43be97` + `b155758` + `bfe9c58`): **Redirect-bounce target convention — `/dashboard` is the dominant target, `/dashboard/not-authorized` exists for explicit access-denied UX — RIPE NOW (6 vs 1 instances split confirmed)**. (1) `/dashboard` target (6 instances): agent-console, workstation, tenants, insurance-claims, audit, fhir-export. (2) `/dashboard/not-authorized` target (1 instance, the explicit access-denied UX): lab-intel (page.tsx:230-239 `router.replace("/dashboard/not-authorized?from=...")` — Issue #179 pattern). CLAUDE.md gotcha #7 lists `/not-authorized` as the canonical target but actual practice splits 6:1 toward `/dashboard`. Test pattern: assert based on the page's actual `router.push/replace` call site — `expect(page).toHaveURL(/\/dashboard$/)` for the dominant case, `expect(page).toHaveURL(/\/dashboard\/not-authorized/)` for the rare Issue #179-style case. Both are valid contracts; the ANTI-pattern is assuming one without checking page.tsx first. **Suggested skill extension** for `/medcore-e2e-spec`: add a redirect-target sub-pattern to the page-shape decision matrix that requires the scaffolder to grep page.tsx for the actual `router.push/replace` target string, then mirror it in the test assertion. **Status: RIPE for promotion** — now 12 instances of redirect-bounce surfaced across 5 cron ticks (`/dashboard` target: agent-console, workstation, tenants, insurance-claims, audit, fhir-export, workspace, billing-patient, tenants-onboarding, reports-custom = 10 instances; `/dashboard/not-authorized` target: lab-intel, visitors = 2 instances — 10:2 ratio, both targets now confirmed across multiple pages); codifying the dual-target convention prevents both the brittle `/not-authorized` default assertion AND the inverse "always /dashboard" assumption.

- 2026-05-05 cron-tick (commits `36b6532` + `e8c648d` + `ce747a3`): **Page-shape "admin-gate placeholder" — third archetype alongside redirect-bounce + universal-access — RIPE (3 instances)**. (1) `apps/web/src/app/dashboard/ai-kpis/page.tsx:296` renders `data-testid="ai-kpis-admin-gate"` for non-ADMIN. (2) `apps/web/src/app/dashboard/ai-fraud/page.tsx:478-487` renders an inline `<ShieldCheck>` "Restricted" placeholder for non-{ADMIN,RECEPTION} via `canRead` flag (page.tsx:382), reuses `ai-fraud-page` testid. (3) `apps/web/src/app/dashboard/leave-management/page.tsx:67-73` renders an inline "Access restricted to administrators." card branch for non-ADMIN. All three pages keep the bundle mounted + outer chrome rendered, but data panels are replaced. CLAUDE.md gotcha #7 codified only 2 archetypes ("VIEW_ALLOWED redirect" + "no client gate, API-only"); this is a confirmed 3rd with 3 instances now. **Inconsistent testid conventions across the 3 instances**: ai-kpis uses a dedicated `<route>-admin-gate` testid (cleanest); ai-fraud reuses outer-wrapper testid + textual "Restricted"; leave-management uses no gate-testid at all (test must distinguish via the "Access restricted" copy). **Suggested skill extension** (strongly worth promoting now — 3 confirming instances): extend `/medcore-e2e-spec` with a "page-shape decision matrix" enumerating all 3 archetypes + a normative recommendation that NEW pages adopting this shape SHOULD use a dedicated `<route>-admin-gate` testid for testability — the ai-fraud and leave-management variants are harder to assert without depending on textual copy. **Status: RIPE for promotion** (3 confirming instances + identical structural fix pattern + concrete testid-naming recommendation; testid-convention drift is itself a meta-finding worth flagging in the skill as a "fix-this-when-you-see-it" guideline).

- 2026-05-05 cron-tick (commits `504c48f` + `3123eb2` + `a809efa` + `ce747a3` + `aaa9ad4` + `de0f396` across waves 17-20): **Backlog framing is sometimes aspirational** — RIPE NOW (5+ wave-confirmed instances across cron ticks). Backlog entries in `docs/E2E_COVERAGE_BACKLOG.md` (especially §5 P-priorities) describe INTENDED UX rather than shipped behaviour. Confirmed cases: (1) `/dashboard/workspace` "config (smoke-visited)" — actual page is a DOCTOR-only personal cockpit, not a config surface (wave 17). (2) `/dashboard/reports` "department + metric filters" — actual page is Daily Collection / Report History 2-tab surface with Generate + Schedule modals (Issue #301, wave 18). (3) ER "overflow → waitlist branching" + "fast-track vs standard" — current `/dashboard/emergency` is a 4-column kanban with no overflow lane shipped (wave 19). (4) HR-ops bulk CSV import + permission matrix UI + role-effective-date + shift-conflict UI — none shipped; backlog framing was aspirational (wave 19). (5) P4 chart "active medication list with start/stop dates" + "medication reconciliation across encounters" + "inline imaging viewer" — none shipped; the actual chart surfaces are TrendSparkline SVGs + Documents-tab Download CTAs (wave 20). (6) P1 billing "edit line-item quantity" via dedicated PATCH + "period-locked invoice" `lockedAt` + "credit-note UI" + "overpayment carry-forward" — none shipped; line-item edit is delete+re-add, period lock is implicit via paymentStatus, credit-notes are API-only (wave 20). **Suggested skill extension** for `/medcore-e2e-spec`: add a "verify-before-scaffold" step that requires the agent to grep page.tsx + the API route file for each scenario in the backlog item BEFORE writing test cases; deferred scenarios get explicit "deferred — UI not shipped" + page.tsx evidence in the spec header AND backlog closure annotation. **API-ahead-of-UI sub-pattern (added wave 26)**: when the backend route ships a fully-validated/authorized/audit-logged endpoint but no UI surfaces it (multiple confirmed cases: prescription-lifecycle wave-21 refill/cancel/reject, insurance-claims-lifecycle wave-22 drawer-sync/appeal, telemedicine-deep wave-26 recording-consent/followup/post-consult-Rx/payment), agents should write **API-contract-pin cases** that assert POST/PATCH body shape via `page.route` stub — locks the contract for the future UI builder rather than fabricating selectors against ghost UI. The pattern is "test the wire, not the widget." **Status: RIPE for promotion** — 8+ confirming instances across 6 cron ticks; the verify-first discipline + API-contract-pin escape valve catches ghost-UI assertions while still leaving regulatory-weight contracts pinned.
