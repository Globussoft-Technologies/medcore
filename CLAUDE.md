...
- 2026-05-05 cron-tick (commit `36b6532`): **Page-shape "admin-gate placeholder" — third archetype alongside redirect-bounce + universal-access** — `apps/web/src/app/dashboard/ai-kpis/page.tsx:296` renders a `data-testid="ai-kpis-admin-gate"` placeholder block for non-ADMIN roles instead of `router.replace("/dashboard/not-authorized")`. The page bundle still loads + chrome still renders, but the data panels are replaced with the gate component. CLAUDE.md gotcha #7 codified two archetypes ("VIEW_ALLOWED redirect" vs "no client gate, API-only"); this is a third — "client gate that renders a placeholder, not a redirect." Test pattern: assert the placeholder testid is visible AND the data-panel testids are absent — NOT `expect(page).toHaveURL("/dashboard/not-authorized")`. **Suggested skill extension**: extend `/medcore-e2e-spec` with a "page-shape decision matrix" sub-section enumerating all 3 archetypes (redirect-bounce / universal-access / admin-gate-placeholder) so future scaffolders pick the right assertion shape immediately. Single instance so far; ripe-when-promoted on a 2nd recurrence (other AI-config pages may follow this pattern).
