...
- 2026-05-05 cron-tick (commit `3beeeaf`): **"eager-include leak in catalog endpoints"** — `apps/api/src/routes/packages.ts` `GET /:id` (catalog detail, no `patientId` FK on `HealthPackage` itself) had an eager `include: { purchases: { patient: { user } } }` block that exposed up-to-10 purchaser identities (name + phone) to ANY PATIENT hitting the catalog endpoint. Fix pattern: strip patient-bearing relations from the include for `req.user.role === "PATIENT"`, keep them for staff. **Suggested skill extension**: extend `/medcore-bola-sweep` with a "catalog vs patient-scoped resource" sub-check — when a resource has no direct `patientId` FK but its relations DO, audit the eager-include shapes. Single-instance so far; ripe-when-promoted if a 2nd instance shows up in the long-tail sweep.
