...
- 2026-05-05 cron-tick (commits `e9e032a` + `d43be97` + `b155758`): **Redirect-bounce target convention — pages bounce to `/dashboard`, NOT `/dashboard/not-authorized`** — confirmed across 5+ pages tested in waves 13-14: agent-console, workstation, tenants, insurance-claims, audit. CLAUDE.md gotcha #7 mentions `/dashboard/not-authorized` as the redirect target, but the actual practice across recently-audited pages is to call `router.push("/dashboard")` (or `router.replace("/dashboard")`). Test pattern: assert `expect(page).toHaveURL(/\/dashboard$/)` or `not.toHaveURL(/\/dashboard\/<route>/)` rather than `/not-authorized`. **Suggested skill extension** for `/medcore-e2e-spec`: add a redirect-bounce sub-pattern to the page-shape decision matrix noting the `/dashboard` (default) target — `/dashboard/not-authorized` is rare and used only for explicit "you don't have access" UX. RIPE-when-promoted: 5 instances surfaced across 2 cron ticks; codifying the target-URL pattern would prevent the brittle assertion `expect(page).toHaveURL("/dashboard/not-authorized")` that several specs would write naïvely.
