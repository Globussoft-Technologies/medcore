...
> HEAD on `main` = `0a91aa3` (`chore(claude): broaden allowlist`). Latest 3-commit code wave: `a5bf725` (A2 sweep continuation — 10 modals / 57 pairs), `9ee446e` (A6 closed — `/users` extracted to dedicated route file), `aaadbeb` (`/admissions/[id]` E2E — closes §2.7 backlog entry). Plus 4 GitHub issues filed (#456 AuditLog tenantId · #457 Tenant FK SetNull · #458 HTML5 constraints — audit comment posted · #459 canX-vs-authorize drift — audit comment posted).
...
| A2 | **Multiple modals render `<label>X</label><input>` without `htmlFor`.** WCAG 2.1 AA gap + breaks Playwright `getByLabel`. Partially fixed: AddMedicine / AddSupplier / AddWard (17 pairs in `ab60593`); extended sweep landed `a5bf725` (10 more modals, 57 pairs across expenses / holidays / budgets / payment-plans / duty-roster / scheduled-reports / walk-in / PatientEditModal / notification-templates / certifications). **~50+ pages still unaudited** per the agent's report — appointments, referrals, prescriptions, leave-management, etc. | `cdea823` `8d3f277` `49d829d` `ab60593` `a5bf725` | Continue sweeping. Could batch as another `/medcore-fanout` wave (3-4 agents each owning a subdir). |
...
| A4 | **HTML5 `<input min/max/required>` constraints race React `setError`.** Browser blocks form submit at the constraint layer before React handler runs, so React-side inline-error rendering never fires. Confirmed on payment-plans; pattern likely repo-wide. **Audit done** — Issue [#458](https://github.com/Globussoft-Technologies/medcore/issues/458) has the concrete report: **37 forms AFFECTED** (top 3: patients/[id] ≥6 sub-forms, admissions/[id] ≥4, pharmacy 3), ~25 PARTIAL (UX inconsistency only), ~15 CLEAN. Recommended path: project-wide `noValidate` + React-only validation (Option B in the issue body), already precedent in login/register/EnquiryForm forms. | `3decc91` (payment-plans validation fix) → Issue [#458](https://github.com/Globussoft-Technologies/medcore/issues/458) audit | Per audit recommendation: add `noValidate` + drop HTML5 constraints across the 37 AFFECTED forms. Could batch as `/medcore-fanout` waves by directory (one agent per dashboard cluster). |
| A5 | **`canX` (client) vs `authorize()` (server) RBAC drift.** Fixed on `/dashboard/expenses` (`0646b0b`). **Audit done** — Issue [#459](https://github.com/Globussoft-Technologies/medcore/issues/459) report: **2 OPEN drift > server (CTA → 403)** — `/lab/[orderId] canAddResults` (DOCTOR+NURSE see CTA, server LAB_TECH+ADMIN), `/medicines canEdit` (DOCTOR sees Edit, PATCH 403). **5 drift < server (UI hides legit access)** — including `/antenatal canCreate` where NURSE is allowed server-side but UI hides CTA. ~28 matched. ~36 total predicate sites audited. | `40673aa` (expenses spec) → `0646b0b` (drift fix) → Issue [#459](https://github.com/Globussoft-Technologies/medcore/issues/459) audit | Resolve each open drift: tighten client (most cases — server is the security boundary), or loosen server (only for the 5 hidden-access cases where the server's intent is broader). Could become a project-level CI lint. |
