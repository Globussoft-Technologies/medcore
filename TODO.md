...
> HEAD on `main` = `75a5ccc`. Latest wave (5 commits, 5-agent fanout): `d76669d` (A4 patients/[id] — 7 sub-forms), `8f9807c` (A4 admissions/[id] — 5 sub-forms), `478325e` (A4 pharmacy+cluster — 6 sub-forms), `d5a4fef` (A5 lab tightened; medicines was audit false-positive), `75a5ccc` (A5 client<server drifts — 4 loosened, 1 documented, 1 server tightened). **A5 effectively CLOSED**; A4 ~half done (18/37 forms). 4 GH issues filed (#456 AuditLog tenantId · #457 Tenant FK SetNull · #458 HTML5 constraints w/ audit · #459 canX drift w/ audit).
...
| A4 | **HTML5 `<input min/max/required>` constraints race React `setError`.** Browser blocks form submit at the constraint layer before React handler runs, so React-side inline-error rendering never fires. **Audit done** ([#458](https://github.com/Globussoft-Technologies/medcore/issues/458)): 37 forms AFFECTED, ~25 PARTIAL, ~15 CLEAN. **Wave 1 fix shipped: 18 forms modernized** — patients/[id] (7 sub-forms `d76669d`), admissions/[id] (5 sub-forms `8f9807c`), pharmacy + insurance-claims + prescriptions + referrals (6 sub-forms `478325e`). All use `noValidate` + React-only validation, matching the login/register precedent. **~19 less-trafficked forms still affected** per the audit's full list. | `3decc91` `d76669d` `8f9807c` `478325e` → Issue [#458](https://github.com/Globussoft-Technologies/medcore/issues/458) | Continue sweeping the remaining ~19 forms — could batch as another 3-4 agent wave by dashboard subdir (appointments / blood-bank / leave-management / etc.). |
