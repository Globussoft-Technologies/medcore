...
**Production state at handoff** (commit `efd42c9` — `fix(admin): Today Snapshot Registered count uses correct table + IST day boundary`):
- ✅ HEAD on `main` = `efd42c9`. Working tree clean. CI Test on the latest commits is in flight at handoff time (efd42c9 + aa63e64 pending; earlier ones cancelled or red on a pre-fix BOLA-test regression that was cleared by `805ef79`).
...
- **3 unfiled bug-bash entries closed** with new code: `.issue-details.txt #37` (seed dedup, `5f784a2`), `#40/#41` (Aspirin flag + deploy.sh pharmacy reseed wiring, `aa63e64`), `#48` (admin Today Snapshot IST/UTC timezone clobber, `efd42c9`).
- **5 unfiled bug-bash entries verified ALREADY FIXED on main** by earlier commits (especially `f2dbb99` Apr 24): `#34` past-slot selectable, `#35` 18:00 freeze, `#36` wards 0/0 + Add Bed 400, `#38` 86 immunizations widget, `#42/#43/#44` billing GST + phone + line-item category, `#1/#2` login Remember Me + password toggle. **The demo box needs a re-seed + redeploy** to surface these fixes; bug-bash was conducted against a stale deploy.
...
## What landed 2026-05-09 — autonomous bug-bash, 17 commits, 17 issues + 4 unfiled bugs closed
