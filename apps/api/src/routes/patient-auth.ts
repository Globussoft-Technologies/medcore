...
// ─── GET /api/v1/patient-auth/_debug/firebase ────────────────────────
//
// TEMP production diagnostic (remove once the prod OTP-login 401 is fixed).
// Returns a NON-SECRET snapshot of why patient sign-in might be failing so it
// can be read straight from the browser console / DevTools without SSH access
// to the server. It NEVER returns the private key or the raw service-account
// JSON — only presence booleans, lengths, the (public) project id, a DB/
// migration probe, and a single human-readable `verdict` classifying the cause
// (env missing / project mismatch / migration / db / ok).
router.get(
  "/_debug/firebase",
  async (_req: Request, res: Response): Promise<void> => {
    const fb = getFirebaseAdminStatus();

    // DB + migration probe — a missing table means migrations didn't run.
    let db: {
      connected: boolean;
      usersTableReachable: boolean;
      userCount: number | null;
      error: string | null;
    } = { connected: false, usersTableReachable: false, userCount: null, error: null };
    try {
      await prisma.$queryRaw`SELECT 1`;
      db.connected = true;
      const count = await prisma.user.count();
      db.usersTableReachable = true;
      db.userCount = count;
    } catch (err) {
    ...
    let verdict: string;
    if (!fb.credentialsJsonSet) {
      verdict =
        "ENV_MISSING: FIREBASE_ADMIN_CREDENTIALS_JSON is not set on this server. " +
        (fb.legacyPathStillSet
          ? "Only the old FIREBASE_ADMIN_CREDENTIALS_PATH is set, which is no longer read. "
          : "") +
        "Set the inline JSON env var and restart — this is why firebase-verify returns 401.";
    } else if (!fb.sdkInitialises) {
      verdict = `FIREBASE_CREDENTIALS_INVALID: credentials are present but the Admin SDK failed to init — ${fb.error}`;
    } else if (fb.projectMismatch) {
      verdict = `PROJECT_MISMATCH: FIREBASE_PROJECT_ID (${fb.projectIdEnv}) != service-account project (${fb.serviceAccountProjectId}). The web client's token won't verify. Align both to the web's NEXT_PUBLIC_FIREBASE_PROJECT_ID.`;
    } else if (!db.connected) {
      verdict = `DB_DOWN: database not reachable — ${db.error}`;
    } else if (!db.usersTableReachable) {
      verdict = `MIGRATION_MISSING: DB connects but the users table isn't reachable (${db.error}). Run prisma migrate deploy.`;
    } else {
      verdict =
        "FIREBASE_OK: server credentials + DB are fine. If login still 401s, the token's project differs from this server's project (web built with a DIFFERENT NEXT_PUBLIC_FIREBASE_PROJECT_ID than the server's), the token expired, or the phone has no account. Check the server log line 'verifyIdToken FAILED' for the exact Firebase code.";
    }

    // NON-SECRET backend env snapshot so you can verify the prod server's
    // actual configuration from the browser. Secrets (DB password, JWT
    // secrets, API keys, the service-account private key) are NEVER included —
    // only presence booleans and masked hosts.
    ...
    const backendEnv = {
      ...
      FIREBASE_ADMIN_CREDENTIALS_JSON_set: !!(
        process.env.FIREBASE_ADMIN_CREDENTIALS_JSON &&
        process.env.FIREBASE_ADMIN_CREDENTIALS_JSON.trim()
      ),
      FIREBASE_ADMIN_CREDENTIALS_JSON_length:
        process.env.FIREBASE_ADMIN_CREDENTIALS_JSON?.trim().length ?? 0,
      FIREBASE_ADMIN_CREDENTIALS_PATH_set:
    ...
    res.json({
      ...
      data: {
        ...
        firebase: fb,
        backendEnv,
        webExpectedProjectHint:
          "Compare backendEnv.FIREBASE_PROJECT_ID (and firebase.effectiveProjectId) with the web app's NEXT_PUBLIC_FIREBASE_PROJECT_ID — they MUST match.",
        db,
      },
      error: null,
    });
