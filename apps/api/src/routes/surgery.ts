...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Issue #754 (2026-05-08): previously a non-existent `patientId`
      // (syntactically valid UUID but no row) fell straight into prisma's
      // FK violation path and surfaced HTTP 500 with a Prisma-shaped error.
      // Validate up front so the caller sees a user-friendly 404 instead.
      ...
      // Issue #753 (2026-05-08): caseNumber is generated by reading the
      // last row's number — racy under concurrency. Two concurrent POSTs
      // can compute the same number and collide on the @unique index.
      // Issue #752 (2026-05-08): the prior implementation let the P2002
      // bubble up via next(err), which leaks `prismaCode`, `modelName`,
      // and `meta.target` to the client. Wrap creation in a small retry
      // loop so we re-roll the case number on collision, then translate
      // any persistent uniqueness failure into a generic 409 with no
      // ORM internals.
      let surgery: Awaited<ReturnType<typeof prisma.surgery.create>> | null = null;
      ...
      for (let attempt = 0; attempt < 3 && !surgery; attempt++) {
        try {
          ...
          lastErr = err;
          // Detect Prisma unique-constraint violation on caseNumber via
          // duck-typed code — avoids leaking the type into our public
          // error surface and works whether the runtime is the real
          // PrismaClient or a mock.
          const code = (err as { code?: string } | null)?.code;
          if (code === "P2002") {
            continue; // retry with a new case number
          }
          throw err;
        }
      }
      if (!surgery) {
        // Issue #752: do NOT pass the P2002 error through next(err) — that
        // path returns the full Prisma payload to the client. Emit a
        // generic 409 instead and log internally.
        console.error("Surgery scheduling failed after 3 retries", lastErr);
        res.status(409).json({
          success: false,
          data: null,
          error: "Could not schedule surgery, please retry",
        });
        return;
      }
