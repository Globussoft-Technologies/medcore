...
export default function PatientLoginPage() {
  ...
  async function verifyCode(): Promise<void> {
    ...
    try {
      ...
      // TEMP production debug (remove once the OTP-login 401 is fixed): the
      // server only returns a generic "Couldn't sign you in", so on ANY verify
      // failure we hit the non-secret /_debug/firebase endpoint and print a
      // CLASSIFIED verdict (env missing / project mismatch / migration / db)
      // straight to the browser console so the cause is visible without server
      // access. Best-effort — never blocks the real error handling below.
      try {
        const e2 = err as { status?: number; message?: string };
        // eslint-disable-next-line no-console
        console.group("%c[MedCore OTP debug] firebase-verify failed", "color:#e11d48;font-weight:bold");
        // eslint-disable-next-line no-console
        console.log("status:", e2?.status, "message:", e2?.message);

        // (a) WEB-side Firebase env — what THIS browser bundle is configured
        // with. API key is non-secret for Firebase web apps; we mask the tail
        // anyway. The projectId here MUST match the server's effectiveProjectId
        // below, or every token will be rejected with a 401.
        const apiKey = process.env.NEXT_PUBLIC_FIREBASE_API_KEY ?? "";
        const webFirebaseEnv = {
          projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID ?? "(unset)",
          authDomain: process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ?? "(unset)",
          apiKeyPresent: !!apiKey,
          apiKeyPreview: apiKey ? `${apiKey.slice(0, 8)}…${apiKey.slice(-4)}` : "(unset)",
          appIdPresent: !!process.env.NEXT_PUBLIC_FIREBASE_APP_ID,
          messagingSenderId: process.env.NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID ?? "(unset)",
          apiBaseUrl: process.env.NEXT_PUBLIC_API_URL ?? "(unset)",
          origin: typeof window !== "undefined" ? window.location.origin : "(ssr)",
        };
        // eslint-disable-next-line no-console
        console.log("%cWEB Firebase env (this browser):", "font-weight:bold", webFirebaseEnv);

        // (b) SERVER-side snapshot from the non-secret debug endpoint —
        // includes the BACKEND ENV the prod server is actually running with.
        const diag = await api
          .get<{
            data: {
              verdict: string;
              firebase: { effectiveProjectId?: string };
              backendEnv: Record<string, unknown>;
              db: unknown;
              nodeEnv: string;
            };
          }>("/patient-auth/_debug/firebase", { skip401Redirect: true })
          .catch((de) => ({
            data: {
              verdict: `(debug endpoint unreachable: ${(de as Error)?.message}) — is the new build deployed?`,
            },
          }));
        const data = (diag as {
          data?: {
        ...
        // eslint-disable-next-line no-console
        console.log("%cBACKEND env (prod server):", "font-weight:bold", data?.backendEnv);
        // eslint-disable-next-line no-console
        console.log("%cBACKEND db probe:", "font-weight:bold", data?.db);
        // eslint-disable-next-line no-console
        console.log("%cSERVER firebase status:", "font-weight:bold", data?.firebase);

        // (c) The decisive comparison — web project vs server project.
        const webProject = webFirebaseEnv.projectId;
        const serverProject = data?.firebase?.effectiveProjectId ?? "(unknown)";
        if (webProject !== "(unset)" && serverProject !== "(unknown)") {
          if (webProject === serverProject) {
            // eslint-disable-next-line no-console
            console.log(`%c✅ PROJECT MATCH: web & server both use "${webProject}"`, "color:#16a34a;font-weight:bold");
          } else {
            // eslint-disable-next-line no-console
            console.error(`❌ PROJECT MISMATCH → THIS is the 401 cause: web="${webProject}" vs server="${serverProject}". Make NEXT_PUBLIC_FIREBASE_PROJECT_ID and the server's FIREBASE_PROJECT_ID / service-account the SAME project.`);
          }
        }
        // eslint-disable-next-line no-console
        console.log("%cVERDICT:", "font-weight:bold", data?.verdict);
        // eslint-disable-next-line no-console
        console.groupEnd();
      } catch {
        /* debug must never break the real flow */
      }
