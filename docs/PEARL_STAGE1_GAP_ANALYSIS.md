...
| 5.1 | Cohort-based audience | 🟡 Partial | `ChronicCarePlan` (auto-enrol by condition); **piece 1 of 4 (#4): Campaign(kind=COHORT_REMINDER) + CampaignAudience schema shipped 2026-05-21** | ChronicCarePlan (per-patient) coexists; tenant-wide rule DSL = piece 2 compiler. |
| 5.1 | Audience builder (demographic + clinical filters) | ❌ Missing | **piece 1 of 4 (#4): CampaignAudience.rules Json placeholder shipped 2026-05-21** | DSL → Prisma `where` compiler = piece 2; UI builder = piece 4. |
...
| 4 | **Build Campaign engine (`Campaign`, `CampaignSend`, `CampaignAudience`, audience builder UI, 4-channel dispatcher, send-window respect, A/B variant)** | 2.5 weeks | 🚧 **In progress — piece 1 of 4 closed 2026-05-21**. Migration `20260520000012_pearl_campaign_engine_schema` adds 4 enums (`CampaignChannel`, `CampaignKind`, `CampaignStatus`, `SendStatus`) + 3 tables (`campaigns`, `campaign_audiences`, `campaign_sends`). Routes `apps/api/src/routes/campaigns.ts` (6 endpoints — POST/GET/GET-detail/PATCH/DELETE soft-cancel/POST :id/sends/preview stub) + `apps/api/src/routes/campaign-audiences.ts` (4 endpoints — POST/GET/GET-detail/PATCH; no DELETE since audiences may be referenced by past campaigns). Zod schemas in `packages/shared/src/validation/campaign.ts` with sendWindow validation + state-machine guard (`OPERATOR_STATUS_TRANSITIONS`). 17 integration tests in `campaigns.test.ts` + 10 in `campaign-audiences.test.ts`. Coexists with `NotificationBroadcast` (one-off blast) and `ChronicCarePlan` (per-patient care plan) — no refactor. **Pending**: piece 2 = audience builder API (compile DSL → patient set) + dispatcher worker + send-window clamp + per-channel template substitution; piece 3 = A/B variant pick + conversion-attribution + tracking-aggregate rollups; piece 4 = Campaigns UI (list / create / audience builder UI / tracking dashboard). Closes M4 §5.1 entirely once all 4 pieces land. Reuses existing `services/channels/*`. |
