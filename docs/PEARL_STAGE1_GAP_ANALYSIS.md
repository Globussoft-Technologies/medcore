...
| 6.2 | Installable PWA | 🟡 Partial | [`apps/web/src/app/manifest.ts`](../apps/web/src/app/manifest.ts), [`apps/web/src/app/patient/manifest.ts`](../apps/web/src/app/patient/manifest.ts) | Staff manifest unchanged (`start_url: "/"`). **piece 1 of 4: scaffold shipped 2026-05-21** — `/patient` route group exists, segment-scoped manifest (`start_url`/`scope` = `/patient`) + vanilla SW (`apps/web/public/sw.js`) registered under scope `/patient`. Per-tenant manifest hydration deferred to piece 3. |
| 6.2 | Offline-tolerant SW caching dashboard + recent records | ❌ Missing | [`apps/web/public/sw.js`](../apps/web/public/sw.js) | **piece 1 of 4: scaffold shipped 2026-05-21** — SW boilerplate registers under scope `/patient` (no-op install/activate; NO `fetch` handler / cache strategy yet — that lands in piece 4). |
