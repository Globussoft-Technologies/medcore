...
| 5 | **Build patient PWA route group `apps/web/src/app/patient/` + service worker + phone-OTP login route** | 3 weeks | 🚧 **In progress — piece 1 of 4 closed 2026-05-21** (`/patient` route group scaffold + dedicated manifest + service worker boilerplate). Layout / landing page / loading / not-found ship under `apps/web/src/app/patient/`; segment-scoped manifest at `apps/web/src/app/patient/manifest.ts` (`start_url`/`scope` = `/patient`); vanilla SW at `apps/web/public/sw.js` registered scope-pinned to `/patient` via `components/PatientServiceWorkerRegistration.tsx`; 4 Vitest assertions covering landing / disabled-CTA / no-dashboard-sidebar / manifest shape. **Pending**: piece 2 = phone-OTP login backend + login page; piece 3 = dashboard + appointments + Rx + bills pages; piece 4 = offline cache. Closes M5 §6.1+6.2 when all 4 land. Backend ~90% reused. |
