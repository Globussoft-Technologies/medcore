...
| 2.1.2 | Three doctor modes (Calling / Token / Slot) configurable per doctor | 🟡 Partial | `Doctor.appointmentMode` + 5 knobs in [`schema.prisma:936-941`](../packages/db/prisma/schema.prisma), [`routes/appointments.ts` branch-by-mode](../apps/api/src/routes/appointments.ts), [`PATCH /doctors/:id/appointment-mode`](../apps/api/src/routes/doctors.ts) | **Piece 1 of 3 closed 2026-05-21**. Schema + Zod + booking handler branching (TOKEN/SLOT/CALLING) + dailyAppointmentLimit cap + lastHourPolicy=BLOCK_NEW gate + tokenPrefix `displayToken` in response. Pending: piece 2 = doctor-profile mode-picker UI (`apps/web/.../doctors/[id]`); piece 3 = token-board variant rendering + Calling-mode arrival-queue UI. |
...
| # | Move | Effort | Why |
|---|---|---|---|
| 1 | **Add `Doctor.appointmentMode` + Calling/Token/Slot routing in `routes/appointments.ts` + token board variant rendering** — 🟡 **piece 1 of 3 closed 2026-05-21** (schema + API foundation + dailyLimit/lastHour/tokenPrefix enforcement, see commits `bfd11a8` + `6913d62` + `e35081b` + this commit). TODO piece 2 = doctor-profile mode-picker UI. TODO piece 3 = token-board variant rendering + Calling-mode arrival-queue UI. | 1 week (0.4w done, 0.6w left) | Single largest functional blocker for Pearl M1+M2 acceptance. Schema + booking flow + display board. |
