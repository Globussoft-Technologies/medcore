...
| 2.1.2 | Three doctor modes (Calling / Token / Slot) configurable per doctor | ❌ Missing | — | **No `Doctor.appointmentMode`**, no Calling-mode arrival-order queue. Today everyone is implicitly Token-mode (every `Appointment` gets a `tokenNumber`). This is the single biggest M1 build. |
| 2.1.2 | Branching UX (OPD header changes per mode) | ❌ Missing | — | Follows from the schema gap above. |
...
| 2.1.4 | Drug-allergy conflict block + duplicate-drug + DDI | ✅ Present | [`routes/prescriptions.ts:checkPatientAllergies`](../apps/api/src/routes/prescriptions.ts), `DrugInteraction` model, `PatientAllergy.active` | **CLOSED 2026-05-21 (gap item #7)**. DDI engine + duplicate-drug detection already shipped (NEW_VS_NEW + NEW_VS_EXISTING). Allergy half now wired: POST/PATCH `/prescriptions` cross-references `PatientAllergy` rows (active=true only) for each Rx item via case-insensitive bidirectional substring on Medicine name/genericName. Block returns 409 with `{allergyConflicts:[{itemIndex,medicineName,allergySubstance,allergySeverity,reaction}]}`. Override path = `allergyOverrideReason` (>=10 chars) on body → persisted on `Prescription.allergyOverrideReason`+`allergyOverrideAt` AND a `PRESCRIPTION_ALLERGY_OVERRIDE` AuditLog row. UI modal in `/dashboard/prescriptions` opens on 409 with per-conflict list + reason textarea. Migration `20260520000008_pearl_rx_allergy_override`. 4 integration tests. |
...
| 2.1.5 | Three layouts in one screen (Calling / Token / Slot) | ❌ Missing | — | Today's display is Token-only. |
...
| 3.2 | Per-doctor preference editor (channels / token prefix / token start / daily limit / slot duration / buffer / hours / near-turn alert / last-hour) | 🟡 Partial | [`/dashboard/duty-roster/`](../apps/web/src/app/dashboard/duty-roster/), [`/dashboard/schedule/`](../apps/web/src/app/dashboard/schedule/) | Working hours + slot duration in admin schedule UI. Other 6 knobs not exposed. |
...
| # | Move | Effort | Why |
|---|---|---|---|
| 1 | **Add `Doctor.appointmentMode` + Calling/Token/Slot routing in `routes/appointments.ts` + token board variant rendering** | 1 week | Single largest functional blocker for Pearl M1+M2 acceptance. Schema + booking flow + display board. |
...
| 7 | ~~**Wire `PatientAllergy` into `routes/prescriptions.ts` allergy-block + override-with-reason path**~~ | ~~2 days~~ | ✅ **CLOSED 2026-05-21**. Migration `20260520000008_pearl_rx_allergy_override` adds `Prescription.allergyOverrideReason`+`allergyOverrideAt` + `PatientAllergy.active` (default true). `checkPatientAllergies()` helper in `routes/prescriptions.ts` cross-references active allergies against each Rx item; POST/PATCH return 409 + `allergyConflicts[]` payload tagged with `itemIndex` so the UI can map back to specific rows. Override path: non-empty `allergyOverrideReason` (>=10 chars) on body → persists on the Rx row AND writes `PRESCRIPTION_ALLERGY_OVERRIDE` AuditLog row with `{allergyConflicts, reason, prescribedBy}`. Web modal in `/dashboard/prescriptions` lists each conflict + textarea + Override-and-Save button. 4 integration tests in `rx-allergy-block.test.ts` (block, override, no-conflict regression, inactive-allergy bypass). |
