...
| 2.1.4 | Drug-allergy conflict block + duplicate-drug + DDI | 🟡 Partial | [`routes/prescriptions.ts:checkDrugInteractions`](../apps/api/src/routes/prescriptions.ts), `DrugInteraction` model | DDI engine + duplicate-drug detection present (NEW_VS_NEW + NEW_VS_EXISTING grouping). `PatientAllergy` model exists but **`prescriptions.ts` does NOT cross-reference it** before signing — Pearl's "block + override-with-reason" gate is partially there for DDIs but NOT for allergies. |
...
| 3 | **Build Lead pipeline (`Lead` model + 6-stage state machine + activity log + convert-to-patient)** | 1.5 weeks | Closes M2 §3.3 entirely. |
