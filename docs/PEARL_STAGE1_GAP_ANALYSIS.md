...
| 6 | **Build super-admin route group `apps/web/src/app/super-admin/` on separate vhost + onboarding wizard + Pearl-billing surface (`PearlSubscription`, `PearlInvoice`)** | 2.5 weeks | 🚧 **In progress — piece 1 of 4 closed 2026-05-21** (`/super-admin` route group scaffold + client-side gate enforcing `Role.ADMIN AND tenantId == null` + bare dark topbar + landing dashboard with Tenants tile + disabled Pearl-Billing placeholder). Layout / page / loading / not-found ship under `apps/web/src/app/super-admin/`; `User.tenantId` surfaced on the web `apps/web/src/lib/store.ts` `User` interface (was missing — the API's `/auth/me` already returned it). 7 Vitest assertions across `__tests__/landing.page.test.tsx` cover happy-path render / 4 gate branches (super-admin OK; tenant-admin redirected; non-ADMIN redirected; unauthenticated → /login) / disabled Pearl-Billing tile / Tenants-tile link target. **Scope-cut**: server-side gate via `apps/web/middleware.ts` deferred to piece 2 — no `middleware.ts` exists in the app today and adding one as a first instance is non-trivial scope; the client-side gate prevents UI flash and the existing `/login` redirect handles direct unauthenticated hits. **Pending**: piece 2 = onboarding wizard (Pearl §8.1 step-by-step new-tenant creation: tenant + first branch + super-admin user + HFR/HPR/WhatsApp/Razorpay config steps) + server-side middleware gate; piece 3 = `PearlSubscription` + `PearlInvoice` schema + Pearl-side billing surface (Pearl → hospital invoice generation, state machine, proration); piece 4 = cross-tenant metrics + per-tenant health + DPDP workbench. Closes M7 §8.1+8.3 when all 4 land. |
